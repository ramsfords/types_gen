...
func (m *Location) validate(all bool) error {
	...
	for idx, item := range m.GetDeliveryLocationServices() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, LocationValidationError{
						field:  fmt.Sprintf("DeliveryLocationServices[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
	...
	for idx, item := range m.GetPickupLocationServices() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, LocationValidationError{
						field:  fmt.Sprintf("PickupLocationServices[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
